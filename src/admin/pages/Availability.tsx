      ...
      <div className="rounded-xl border border-white/5 bg-surface-elevated divide-y divide-white/5">
        {rows.map((row) => (
          <div key={row.id} className="flex items-center gap-6 p-5 flex-wrap">
            <span className="w-32 font-heading text-lg">{WEEKDAY_LABELS[row.weekday]}</span>
            <button
              onClick={() => update(row, { is_available: !row.is_available })}
              className={`rounded-full px-4 py-2 text-base border focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold-light focus-visible:outline-offset-2 ${row.is_available ? 'border-emerald-500/40 text-emerald-300' : 'border-white/15 text-muted'}`}
            >
              {row.is_available ? 'Beschikbaar' : 'Niet beschikbaar'}
            </button>
            <label className="flex items-center gap-2 text-base text-muted">
              Van
              <input type="time" value={row.start_time.slice(0, 5)} onChange={(e) => update(row, { start_time: e.target.value })} className="bg-surface border border-white/15 rounded-lg px-3 py-1.5 text-base text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold-light" />
            </label>
            <label className="flex items-center gap-2 text-base text-muted">
              Tot
              <input type="time" value={row.end_time.slice(0, 5)} onChange={(e) => update(row, { end_time: e.target.value })} className="bg-surface border border-white/15 rounded-lg px-3 py-1.5 text-base text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold-light" />
            </label>
          </div>
        ))}
      </div>
