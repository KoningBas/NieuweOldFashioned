    ...
    <AdminLayout title="Servicepakketten">
        ...
        {packages.map((pkg) => (
          <div key={pkg.id} className="rounded-xl bg-surface-elevated border border-white/5 p-6">
            <div className="flex justify-between items-start mb-3">
              <h3 className="font-heading text-lg">{pkg.package_name}</h3>
              <button onClick={() => openEdit(pkg)} className="text-sm text-gold-light hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold-light focus-visible:outline-offset-2 rounded">Bewerken</button>
            </div>
            <p className="text-muted text-sm mb-4">{pkg.description}</p>
            <div className="text-gold-light mb-4">&euro;{pkg.price} {pkg.price_unit === 'per_cocktail' ? 'per cocktail' : 'per persoon'} &middot; min. {pkg.min_quantity}</div>
            <div className="flex gap-2">
              <button onClick={() => toggle(pkg, 'is_active')} className={`rounded-full px-3 py-1 text-xs border focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold-light focus-visible:outline-offset-2 ${pkg.is_active ? 'border-emerald-500/40 text-emerald-300' : 'border-white/15 text-muted'}`}>
                {pkg.is_active ? 'Actief' : 'Inactief'}
              </button>
              <button onClick={() => toggle(pkg, 'is_featured')} className={`rounded-full px-3 py-1 text-xs border focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold-light focus-visible:outline-offset-2 ${pkg.is_featured ? 'border-gold/40 text-gold-light' : 'border-white/15 text-muted'}`}>
      ...
      <Drawer open={editing !== null || creating} title={editing ? 'Pakket bewerken' : 'Nieuw pakket'} onClose={close}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <label className="block">
            <span className="block text-sm text-muted mb-2">Prijseenheid</span>
          ...
          <label className="block">
            <span className="block text-sm text-muted mb-2">Prijs (&euro;)</span>
          ...
          <label className="block">
            <span className="block text-sm text-muted mb-2">Minimale afname</span>
