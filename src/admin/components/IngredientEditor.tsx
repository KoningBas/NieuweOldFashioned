...
//
// Rows write themselves back shortly after you stop typing, one row per write.
// Deleting is immediate — the undo toast puts the row back by its own id rather
// than holding a delete in limbo, so the table always matches the database.
...
export function IngredientEditor({ cocktailId }: { cocktailId: string }) {
  ...
  async function removeRow(row: CocktailIngredient) {
    saver.forget(row.id);
    setRows((prev) => prev.filter((r) => r.id !== row.id));

    const { error: err } = await supabase.from('cocktail_ingredients').delete().eq('id', row.id);
    if (err) {
    ...
    undo.offer(`${row.name || 'Ingrediënt'} verwijderd`, async () => {
      const { data, error: backErr } = await supabase.from('cocktail_ingredients').insert(row).select().single();
              ...
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-white/5 last:border-b-0">
                  <td className="px-2 py-1">
                    <input aria-label="Naam" value={row.name} onChange={(e) => patch(row.id, { name: e.target.value })} className={CELL_INPUT_CLS} placeholder="Bourbon" />
                  </td>
                  <td className="px-2 py-1">
                    <input aria-label="Hoeveelheid per cocktail" type="number" step="0.01" inputMode="decimal" value={row.amount} onChange={(e) => patch(row.id, { amount: Number(e.target.value) })} className={`${CELL_INPUT_CLS} text-right`} />
                  </td>
                  <td className="px-2 py-1">
                    <select aria-label="Eenheid" value={row.unit} onChange={(e) => patch(row.id, { unit: e.target.value as CocktailIngredient['unit'] })} className={CELL_INPUT_CLS}>
                  ...
                  <td className="px-2 py-1">
                    <input aria-label="Verpakkingsinhoud" type="number" step="0.01" inputMode="decimal" value={row.pack_size ?? ''} placeholder="–" onChange={(e) => patch(row.id, { pack_size: e.target.value === '' ? null : Number(e.target.value) })} className={`${CELL_INPUT_CLS} text-right`} />
                  </td>
                  <td className="px-2 py-1">
                    <input aria-label="Inkoopeenheid" value={row.pack_unit ?? ''} placeholder="–" onChange={(e) => patch(row.id, { pack_unit: e.target.value || null })} className={CELL_INPUT_CLS} />
                  </td>
