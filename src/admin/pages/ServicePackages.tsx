...
import { deletePackage, setActive, usageSentence } from '../lib/deletion';
...
/** What the delete dialog is currently asking. `blocked` arrives after the
 *  reference count comes back and turns the question into archiving. */
interface Removal {
  pkg: ServicePackage;
  blocked: number | null;
}
...
export function ServicePackages() {
  ...
  function openForm(pkg: ServicePackage | null) {
    const next: FormState = pkg
      ? { package_name: pkg.package_name, description: pkg.description, price: pkg.price, price_unit: pkg.price_unit, min_quantity: pkg.min_quantity, category: pkg.category, is_featured: pkg.is_featured, is_active: pkg.is_active }
      : EMPTY_FORM;
    setEditing(pkg);
    setCreating(pkg === null);
    setForm(next);
    setBaseline(next);
  }
  ...
  /** The panel is the one place that still saves on command, so closing it with
   *  unsaved edits has to ask rather than swallow them. */
  ...
  async function confirmRemoval() {
    ...
    if (removal.blocked !== null) {
      const message = await setActive('service_packages', removal.pkg.id, false);
      setRemoving(false);
    ...
    const outcome = await deletePackage(removal.pkg.id);
    setRemoving(false);
  ...
  return (
    <SaveStatusProvider>
      <AdminLayout title="Pakketten">
                ...
                <Disclosure
                  ...
                  actions={
                    <>
                      ...
                      <button
                        onClick={() => setRemoval({ pkg, blocked: null })}
                        aria-label={`Verwijder pakket ${pkg.package_name}`}
        ...
        <Drawer open={drawerOpen} title={editing ? 'Pakket bewerken' : 'Nieuw pakket'} onClose={requestClose}>
        ...
        >
          Je hebt dit pakket aangepast zonder op te slaan. Sluiten gooit die wijzigingen weg.
        </ConfirmDialog>

        <ConfirmDialog
          open={removal !== null}
          title={removal?.blocked ? 'Dit pakket kan niet weg' : 'Pakket verwijderen?'}
        ...
        >
          {removal?.blocked
            ? usageSentence('service_packages', removal.pkg.package_name, removal.blocked)
            : `“${removal?.pkg.package_name}” verdwijnt van de site en uit de offerte-wizard, samen met zijn paklijstsjabloon. Dit kan niet ongedaan gemaakt worden.`}
        </ConfirmDialog>
      </AdminLayout>
    </SaveStatusProvider>
  );
